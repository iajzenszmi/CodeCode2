...
C README.DOC
C 
CThis disk accompanies the submission "Fortran codes for estimating the one-
Cnorm of a real or complex matrix, with applications to condition estimation"
Cto ACM Trans. Math. Soft.
C 
CThe files are:
C 
CSONRUN.FOR          Test driver program for subroutine SONEST.
CRAND.FOR            Random number generators.
CSONEST.FOR          Subroutine SONEST.
CLPSTUFF.FOR         LINPACK SGE?? routines (for real matrices)
C                    and associated BLAS routines.
C 
C 
CONRUN.FOR          Test driver program for subroutine CONEST.
CONEST.FOR          Subroutine CONEST.
CLPCMPLX.FOR         LINPACK CGE?? routines (for complex matrices)
C                    and associated BLAS routines.
CMYCBLAS.FOR         Modified complex BLAS routines.
C 
C 
CThe dependency among these files is as follows:
C 
CSONRUN.FOR uses the subroutines and functions contained in
CRAND.FOR, SONEST.FOR and LPSTUFF.FOR
C 
CCONRUN.FOR uses the subroutines and functions contained in
CRAND.FOR, CONEST.FOR, LPCMPLX.FOR and MYCBLAS.FOR
C 
C 
CNicholas J. Higham           na.higham@score.stanford.edu
CUniversity of Manchester
CManchester M13 9PL
CEngland
C 
C
...
C
C  (1)  ILLUSTRATES HOW TO USE CONEST (WITH LINPACKS CGEFA/CGESL).
C
...
C  U  AND  V  ARE BOTH THE PRODUCT OF NHTS RANDOM HOUSEHOLDER MATRICES.
      PARAMETER (NHTS = 5)
C  IN THIS ROUTINE WE MAKE USE OF FORTRANS IMPLICIT REAL TO COMPLEX
C  CONVERSIONS.
   ...
   25 CONTINUE
C  TRANSPOSE A (DONT NEED TO CONJUGATE)
...
C
   ...
   60 SDOT = STEMP
      ...
      END
C
C   NEXT BLAS IS NOT FROM ABOVE NETLIB MESSAGE.  ITS ONE I MODIFIED
